...
//   const [engagements, setEngagements] = useState<Engagement[]>([]);
//   // Use explicit type or allow flexible structure to avoid runtime errors if API changes
//   const [summary, setSummary] = useState<{ totalHours?: number; totalAmount?: number; count?: number } | null>(null);
//   const [loading, setLoading] = useState(false);
//   const [generating, setGenerating] = useState(false);
...
//     if (isOpen) {
//         apiClient.getEngagements({ page: 1, limit: 100 }).then(res => {
//             if (res.data && res.data.engagements) {
//                 setEngagements(res.data.engagements);
//             }
//         });
//         // Set default dates
...
//                             <ul className="mt-2 space-y-1 text-blue-800">
//                                 <li>Entries: {count}</li>
//                                 {/* FIX: Safely handle undefined/null with default value before toFixed */}
//                                 <li>Hours: {Number(hours).toFixed(2)}</li>
//                                 <li className="text-lg font-bold mt-2">Total: ${Number(amount).toLocaleString()}</li>
//                             </ul>
//                         </div>
//                     ) : (
//                         <p className="text-sm text-gray-500">No unbilled hours found for this engagement.</p>
...
//                 <div>
//                     <label className="block text-sm font-medium text-gray-700">Issue Date</label>
//                     <input type="date" {...register('issueDate')} className="mt-1 block w-full rounded-md border border-gray-300 p-2" />
//                 </div>
//                 <div>
//                     <label className="block text-sm font-medium text-gray-700">Due Date</label>
//                     <input type="date" {...register('dueDate')} className="mt-1 block w-full rounded-md border border-gray-300 p-2" />
//                 </div>
//             </div>

//             <button
//                 type="submit"
//                 disabled={generating || !summary || count === 0}
//                 className="w-full mt-4 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
//             >
//                 {generating ? <LoadingSpinner size="sm" color="white" /> : 'Generate Invoice'}
//             </button>
//           </form>
...
// src/pages/Billing/GenerateInvoiceModal.tsx
...
const GenerateInvoiceModal: React.FC<Props> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [engagements, setEngagements] = useState<Engagement[]>([]);
  // Updated state type to match working API fields
  const [summary, setSummary] = useState<{ totalHours?: number; unbilledAmount?: number } | null>(null);
  const [loading, setLoading] = useState(false);
  const [generating, setGenerating] = useState(false);
  ...
  useEffect(() => {
    if (isOpen) {
        apiClient.getEngagements({ page: 1, limit: 100 }).then(res => {
            if (res.data?.engagements) setEngagements(res.data.engagements);
        });
        setValue('issueDate', new Date().toISOString().split('T')[0]);
        const due = new Date();
        due.setDate(due.getDate() + 30);
  ...
  // Map fields from the working API response
  const hours = summary?.totalHours || 0;
  const amount = summary?.unbilledAmount || 0;
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl p-6 shadow-xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className={`p-4 rounded-lg border ${loading ? 'bg-gray-50' : 'bg-blue-50 border-blue-200'}`}>
                    {loading ? <LoadingSpinner size="sm" /> : summary ? (
                        <div className="text-sm">
                            <p className="font-semibold text-blue-900">Unbilled Work Found:</p>
                            <ul className="mt-2 space-y-1 text-blue-800">
                                <li>Hours to be billed: {Number(hours).toFixed(2)}</li>
                                <li className="text-lg font-bold mt-2">Invoice Total: ${Number(amount).toLocaleString()}</li>
                            </ul>
                        </div>
                    ) : (
                        <p className="text-sm text-gray-500">No unbilled hours found for this engagement.</p>
            ...
            <button
                type="submit"
                // Check hours or amount instead of 'count' which isn't in the API
                disabled={generating || !summary || hours === 0}
                className="w-full mt-4 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
            >
                {generating ? <LoadingSpinner size="sm" color="white" /> : 'Generate Invoice'}
            </button>
