...
const TimeEntryModal: React.FC<Props> = ({ isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      // FIX: Use pagination params instead of status='active' to avoid 400 Bad Request
      apiClient.getEngagements({ page: 1, limit: 100 })
        .then(res => {
          // Handle response structure { data: { engagements: [] } }
          if (res.data && Array.isArray(res.data.engagements)) {
            setEngagements(res.data.engagements);
          } else if (Array.isArray(res.data)) {
            // Fallback if API returns array directly
            setEngagements(res.data as any);
          }
        })
        .catch(err => {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <select 
                ...
                className="mt-1 block w-full rounded-md border border-gray-300 p-2 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              >
                <option value="">Select Engagement...</option>
                {engagements.map(e => (
