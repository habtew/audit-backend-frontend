...
import { TrialBalanceAccount, AdjustmentPayload } from '../../types';
...
const AdjustmentsModal: React.FC<Props> = ({ isOpen, onClose, onSuccess, tbId, account }) => {
  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<AdjustmentPayload>();
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl p-6 shadow-xl">
          <div className="flex justify-between items-center mb-4">
            <Dialog.Title className="text-lg font-bold">Adjust: {account.accountName}</Dialog.Title>
