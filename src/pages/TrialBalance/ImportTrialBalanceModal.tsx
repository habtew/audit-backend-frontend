...
//       // FIX: Ensure we are passing the first file from the FileList
...
//       await apiClient.importTrialBalance({
//         file: fileToUpload,
//         engagementId: data.engagementId, // Ensure this ID is valid
//         period: new Date(data.period).toISOString(), // Ensure ISO format for backend DTO
//         description: data.description
//       });
...
//       // Show the actual error from backend (e.g., "Invalid file format")
...
// src/pages/TrialBalance/ImportTrialBalanceModal.tsx

import React, { useState } from 'react';
import { useForm } from 'react-hook-form';
import { Dialog } from '@headlessui/react';
import { XMarkIcon, CloudArrowUpIcon } from '@heroicons/react/24/outline';
import api from '../../utils/api'; // Ensure this imports your instance
import toast from 'react-hot-toast';
...
const ImportTrialBalanceModal: React.FC<Props> = ({ isOpen, onClose, onSuccess, engagementId }) => {
  ...
  const onSubmit = async (data: ImportForm) => {
    ...
    try {
      ...
      // 2. Prepare Payload
      // Note: We access data.file[0] because useForm returns a FileList
      await api.importTrialBalance({
        file: data.file[0], 
        engagementId: engagementId,
        period: new Date(data.period).toISOString(), // Ensure ISO format for backend
        description: data.description
      });
      ...
      // --- CRITICAL FIX FOR REACT CRASH ---
      // Safely extract the error message from the NestJS response
      ...
      if (error?.response?.data) {
        ...
        if (Array.isArray(backendMessage)) {
          // If validation array (e.g. ["period must be date", "file is required"])
          errorMessage = backendMessage[0]; 
        } else if (typeof backendMessage === 'string') {
          errorMessage = backendMessage;
        } else if (typeof backendMessage === 'object') {
          // If it's a nested object (rare but possible in some NestJS filters)
